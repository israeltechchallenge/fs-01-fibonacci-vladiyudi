...
async function runServerFibonacci() {
  finalResult.classList.add("spinner-border", "text-primary");
  let response = await fetch(`http://localhost:5050/fibonacci/${input.value}`)
  try {
    if (response.ok){
    ...
    finalResult.classList.add("text-danger");
    finalResult.innerText = "Server Error: "+err;
  }
}
function clearInput() {
  input.value = " ";
...
async function fetchResults(selectedValue) {
  spinnerResult.classList.remove("d-none");
  let output = await fetch("http://localhost:5050/getFibonacciResults");
